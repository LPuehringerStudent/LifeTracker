...
export class ApiService {
  ...
  get<T>(path: string, headers?: HttpHeaders): Observable<T> {
    return this.http.get<T>(`${this.baseUrl}${path}`, { headers }).pipe(catchError(err => this.handleError(err)));
  }
  ...
  post<T>(path: string, body: unknown, headers?: HttpHeaders): Observable<T> {
    ...
    return this.http.post<T>(`${this.baseUrl}${path}`, body, { headers: defaultHeaders }).pipe(catchError(err => this.handleError(err)));
  }

  patch<T>(path: string, body: unknown, headers?: HttpHeaders): Observable<T> {
    const defaultHeaders = headers ?? new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.patch<T>(`${this.baseUrl}${path}`, body, { headers: defaultHeaders }).pipe(catchError(err => this.handleError(err)));
  }

  delete<T>(path: string, headers?: HttpHeaders): Observable<T> {
    return this.http.delete<T>(`${this.baseUrl}${path}`, { headers }).pipe(catchError(err => this.handleError(err)));
  }
}
